...
databasePath = os.path.join(dirname, "./Simple_Registration_Database.xlsx")
registrationPath = os.path.join(dirname, "./Registration_UserName_Password.xlsx")
imgPath = os.path.join(dirname, "./BackgroundPicture.png")
vaccinePath = os.path.join(dirname,"./COVID_Vaccine_Database.csv")
# open existing excel files for user infro
# filesshould be saved in the same directory ot path
wb = load_workbook(databasePath)
wb2 = load_workbook(registrationPath)
...
def excel():
    ...
    sheet.cell(row=1, column=12).value = "StudentId"
    sheet.cell(row=1, column=13).value = "School"
    sheet.cell(row=1, column=14).value = "Sex"
...
def current_date_check():
    ...
    Label(
        date_check_screen,
        text="Submit Date To Ckeck System!(format: YYYY-MM-DD)",
        font=("Calibri", 13),
    ).grid(row=0, column=0, padx=25)
...
def first_vaccine():
    ...
    message = """Subject: First COVID-19 Vaccination Recived

    Hello {First} {Last} you recived your first COVID-19 Vaccination today on {Vaccination1} \n

    Your second dose for your complete Vaccincation is on {Vaccination2}
    """

    from_address = "juxsta@jinocenc.io"
    password = "e7@8@cTTTIxw"
        ...
        for i, r in df2.iterrows():
            ...
            if (
                ...
                if r["Sent"] == 1:
                    ...
                    df2.loc[i, "Sent"] = 0
                    df2.to_excel(registrationPath, index=False)
                    df2.to_csv(databasePath, index=None, header=True)
...
def date_verify():
    ...
    from_address = "ee104.jose.r@gmail.com"
    password = "dyllhrmmahxvzxwu"
...
def insert():
    ...
    if (
        ...
        sheet.cell(row=current_row + 1, column=12).value = studentid_field.get()
        sheet.cell(row=current_row + 1, column=13).value = school_field.get()
        sheet.cell(row=current_row + 1, column=14).value = sex_field.get()
        ...
        df_csv = pd.read_excel(databasePath)
        df_csv.to_csv(databasePath, index=None, header=True)
...
def register_form():
    ...
    bg = PhotoImage(file=imgPath, height=400, width=1000)
    ...
    studentid = Label(root, text="studentid", bg="black", fg="white", font=("Calibri", 13))
    school = Label(root, text="school", bg="black", fg="white", font=("Calibri", 13))
    sex = Label(root, text="sex", bg="black", fg="white", font=("Calibri", 13))
    ...
    studentid.grid(row=9, column=0)
    school.grid(row=10, column=0)
    sex.grid(row=11, column=0)
    ...
    studentid_field = Entry(root, textvariable=studentid_in)
    school_field = Entry(root, textvariable=school_in)
    sex_field = Entry(root, textvariable=sex_in)
    ...
    school_field.grid(row=9,column=1,ipadx="100")
    studentid_field.grid(row=10,column=1,ipadx="100")
    sex_field.grid(row=11,column=1,ipadx="100")
...
def register():
    ...
    # refrenced above
    bg = PhotoImage(file=imgPath, height=420, width=300)
...
def login():
    ...
    bg = PhotoImage(file=imgPath, height=400, width=320)
...
def main_account_screen():
    ...
    bg = PhotoImage(file=imgPath, height=400, width=800)
